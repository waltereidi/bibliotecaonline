...
class Mensagens extends Model
{
    ...
    public function getMensagensCaixa(int $meuPerfilId): ?object
    {
        return $this
            ...
            ->select(
                DB::raw('distinct( livros.id ) as livros_id'),
            ...
            ->whereRaw(
                'mensagens.visivel AND ( mensagens.meuperfil_id = ?  OR meuperfil.id = ? ) ',
    ...
    public function getMensagensLivros(array $dados): ?object
    {
        return $this->leftJoin('livros', 'livros.id', '=', DB::raw($dados['livros_id']))
            ->select(
                ...
                'mensagens.livros_id as livros_id',
                'mensagens.visualizado as visualizado',
                'mensagens.created_at as created_at',
                'mensagens.meuperfil_id as meuperfil_id',
                'mensagens.id as id',
            )
            ->whereRaw(
                'mensagens.visivel AND mensagens.livros_id = ? AND ( (mensagens.meuperfil_id = ? OR livros.users_id = ? ) )  ',
    ...
    public function editarMensagensVisualizado(array $dados): bool
    {
        ...
        if ($meuPerfil->id == $dados['sessao_meuperfil_id']) {
            $mensagens = $this->whereRaw(
                ' ( meuperfil_id = ?  AND livros_id = ?  ) AND ( meuperfil_id <> ? ) ',
